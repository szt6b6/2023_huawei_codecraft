			...
			if (map_data[i][j] == '#') { //将障碍物周围2格代价设大点
				for (int row = i - 2; row < i + 2; row++) {
					for (int col = j - 2; col < j + 2; col++) {
						...
						if (dist2 <= 1)
							map_data_cost[row][col] = std::max(map_data_cost[row][col], 5.0); //先设置个5看看 15目前效果比较好
						else if (dist2 <= 2)
							map_data_cost[row][col] = std::max(map_data_cost[row][col], 3.0);
						else if (dist2 <= 3)
							map_data_cost[row][col] = std::max(map_data_cost[row][col], 1.0);
					}
				}
			}
