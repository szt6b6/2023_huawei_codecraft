...
//优先级高的机器人查自己前进路径上有没有其他机器人 如果有 则要慢下来
bool check_other_in_forward_path(int c_robot_id) {
	if (c_robot_id == -1) return false;
	std::list<Node_c> path = working_robots_queue[c_robot_id].path;
	Point c_position = { robot.grid[c_robot_id][0], robot.grid[c_robot_id][1] };

	for (Node_c p : path) {
		for (int r_id = 0; r_id < 4; r_id++) {
			if (r_id == c_robot_id) continue;
			Point o_position = { robot.grid[r_id][0], robot.grid[r_id][1] };
			if ((o_position.x - c_position.x) * (o_position.x - c_position.x) + (o_position.y - c_position.y) * (o_position.y - c_position.y) > 100) continue;

			double dist = (o_position.x - p.point.x) * (o_position.x - p.point.x) + (o_position.y - p.point.y) * (o_position.y - p.point.y);

			if (dist <= 8) return true; //两格内有其他机器人 说明有要慢下来了
		}
	}
	return false;
}
...
void out_of_way_robot(int r_id, int higher_r_id) {
	...
	for (Node_c p : working_robots_queue[higher_r_id].path) {
		for (int i = p.point.x - 3; i <= p.point.x + 3; i++) {
			for (int j = p.point.y - 3; j <= p.point.y + 3; j++) {
	...
	std::list<Point> search_position;
	...
	while (limited_spred_num > 0 && added_node.size() > 0) {
		...
		for (int i = 0; i < 4; i++) {
			if (next_points[i].x < 0 || next_points[i].x > 99 || next_points[i].y < 0 || next_points[i].y > 99) continue;
			...
			if (dis <= 4 || map_data_ob_expand_buy[next_points[i].x][next_points[i].y] == '#' || is_added[next_points[i].x][next_points[i].y]) continue;
			search_position.push_back(next_points[i]);
	...
	if (search_position.size() > 0) {
		...
		if(new_path.size() > 0) working_robots_queue[r_id].path = new_path;
	}


	while (search_position.size() > 0) {
		Point current_p = search_position.front();
		search_position.pop_front();
