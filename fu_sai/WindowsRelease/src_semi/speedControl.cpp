...
void calculateRobotState()
{
	for (int i = 0; i < RobotNum; i++)
	{
		...
		//机器人所处格子x,y方向地图序号
		robot.grid[i][1] = int((robot.local[i][0] + 0.25) / 0.5);
		robot.grid[i][0] = 99 - int((robot.local[i][1] + 0.25) / 0.5);
...
void linSpeedControl(int robotid)
{
	...
	if (1)
	{
			...
			//这里的考虑速度为负情况，得进一步优化
		{
			...
			//setLineSpeed[robotid] = aimDist[robotid] * 10- 20 * angleErr[robotid] * angleErr[robotid];
			if (setLineSpeed[robotid] < 0)setLineSpeed[robotid] = 0;
		}
		//与偏差角越小速度越快
...
void getAimPoint(double *aimPoint, int robotID)
{
	...
	{
		//先找不丢线且道路宽度异常行
		if (findLineFlag[robotID][j] == 1 && roadWidth[robotID][j] <= 2.5&& roadWidth[robotID][j + 1] - roadWidth[robotID][j] >= 1)
		{
			realControlRow = j;
			break;
		}
	...
	for (int k = 1; k < endRow; k++)
	{
		...
		{
			realControlRow = minI(k - 1, realControlRow);//不丢线和改变方向取最小
			break;
		}
		if (k == endRow - 1)
			realControlRow = minI(k - 1, realControlRow);
	}

	//检测长直道
	int temRow = 0;
	while (temRow + 3 < endRow && (roadWidth[robotID][temRow] == roadWidth[robotID][temRow + 1] && roadDir[robotID][temRow] == roadDir[robotID][temRow + 1]
		&& roadWidth[robotID][temRow + 1] == roadWidth[robotID][temRow + 2] && roadDir[robotID][temRow + 1] == roadDir[robotID][temRow + 2]
		&& roadWidth[robotID][temRow + 2] == roadWidth[robotID][temRow + 3] && roadDir[robotID][temRow + 2] == roadDir[robotID][temRow + 3])
		)
		temRow += 1;
	realControlRow = maxI(realControlRow, temRow);
	/*for (int k = 1; k < endRow; k++)
	{
		if (findLineFlag[robotID][k] == 1 && roadWidth[robotID][k] <= 2)
		{
			while (findLineFlag[robotID][k] == 1 && roadWidth[robotID][k] <= 2&& k < endRow)
			{
				k++;
			}
			realControlRow = k - 1;
	...
	while (!judgeHasBorder(robotID, 2, noObLine))
	{
		noObLine += 1;
		if (noObLine >= endRow - 1)
			break;
	}
	realControlRow = maxI(realControlRow, noObLine - 1);
	int narrowLine = 0;//最多的宽赛道的行
	while (roadWidth[robotID][narrowLine] >= 3.5)
	{
		narrowLine += 1;
		if (narrowLine >= endRow - 1)
			break;
	}
	realControlRow = maxI(realControlRow, narrowLine);
	//while (roadWidth[robotID][realControlRow] >= 3)
	//	realControlRow += 1;
...
void avGridDirControl(int robotid)
{
	...
	if (gridCollFlag[robotid] == 0)
	{
		...
		setAngleSpeed[robotid] = -(direction_Kp * error1[robotid] + 0.1*direction_Kd * (error1[robotid] - error2[robotid]));
	}
}
/**
...
void speedControl() {
	...
	for (int i = 0; i < RobotNum; i++)
	{
		...
		if (aimDist[i] <= 2.4&&leaveGridNum[i] <= 5)
		{
			...
			//清0标志位
			aimPoint[0] = workDesk.local[aimDesk[i]][0];
			aimPoint[1] = workDesk.local[aimDesk[i]][1];
			if (leaveGridNum[i] >= 3)
			{
				//得更新roadDir方向
				findBorderLine(i);
			...
			//更新工作台相关参数
			aimDist[i] = calculateDist(robot.local[i], workDesk.local[aimDesk[i]]);
			getAimPoint(aimPoint, i);
			avoidGrid(i);
		}
		if (gridCollFlag[i] == 0)
		{
			Direction_Control_Error(i, aimPoint);
			linSpeedControl(i);
		}
	}
}
