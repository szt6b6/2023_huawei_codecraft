...
void findBorderLine(int robotID)
{
	...
	if (higher_r_id == -1 && check_other_in_forward_path(robotID)) {
	...
	if (realBoderGridNum[robotID] > 2)
	{
		for (int i = 0; i < realBoderGridNum[robotID]; i++)
		{
			...
			if (roadDir[robotID][i] == 1 || roadDir[robotID][i] == 3)
			{
				...
				double upcount1, downcount1;
				//两边都没丢线
				if (upcount < borderLineWidth && downcount < borderLineWidth)
				{
					findLineFlag[robotID][i] = 1;
					upcount1 = double(upcount);
					downcount1 = double(downcount);
			...
			else if (roadDir[robotID][i] == 2 || roadDir[robotID][i] == 4)
			{
				...
				double rightcount1 = 0.0, leftcount1 = 0.0;
				//两边都没丢线
				if (rightcount < borderLineWidth && leftcount < borderLineWidth)
				{
					findLineFlag[robotID][i] = 1;
					rightcount1 = double(rightcount);
					leftcount1 = double(leftcount);
			...
			else if (roadDir[robotID][i] == 5 || roadDir[robotID][i] == 7)
			{
				...
				double rightdowncount1, leftupcount1;
				//两边都没丢线
				if (leftupcount < borderLineWidth && rightdowncount < borderLineWidth)
				{
					findLineFlag[robotID][i] = 1;
					rightdowncount1 = double(rightdowncount);
					leftupcount1 = double(leftupcount);
				...
				//只有一边丢线了
					...
					if (roadDir[robotID][i] == 5)
					{
						...
						midPoint[robotID][i][1] = (-rightdowncount1 + leftupcount1) / 2.0 * 0.5 + (99 - temGridList[robotID][i][0]) * 0.5 + 0.25;
						midPoint[robotID][i][0] = (rightdowncount1 - leftupcount1) / 2.0 * 0.5 + temGridList[robotID][i][1] * 0.5 + 0.25;
					}
					else
					{
						midPoint[robotID][i][1] = (-rightdowncount1 + leftupcount1) / 2.0 * 0.5 + (99 - temGridList[robotID][i][0]) * 0.5 + 0.25;
			...
			else if (roadDir[robotID][i] == 6 || roadDir[robotID][i] == 8)
			{
				...
				double rightupcount1, leftdowncount1;
				//两边都没丢线
				if (leftdowncount < borderLineWidth && rightupcount < borderLineWidth)
				{
					findLineFlag[robotID][i] = 1;
					rightupcount1 = double(rightupcount);
					leftdowncount1 = double(leftdowncount);
					...
					if (roadDir[robotID][i] == 6)
					{
						...
						midPoint[robotID][i][0] = (rightupcount1 - leftdowncount1) / 2.0 * 0.5 + temGridList[robotID][i][1] * 0.5;
					}
					else
					{
						midPoint[robotID][i][1] = (rightupcount1 - leftdowncount1) / 2.0 * 0.5 + (99 - temGridList[robotID][i][0]) * 0.5 - 0.25;
