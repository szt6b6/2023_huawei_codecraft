...
class Config:
    def __init__(self) -> None:
        ...
        # Discretization
        self.dt = 0.02
        self.v_res = 0.4  # Linear velocity resolution
...
def update_queue(workbenchs):
    ...
    for bench_type in range(1, 7): # 不买7 效果似乎好很多
    ...
    for r_id in range(4):
        ...
        if(len(task_queue) and free_robots[r_id]):
            ...
            # #总是以追求单位时间最大金钱的效果不好 效果是真不好
            can_get_profit = 0
            # t = None
            # for task in task_queue:
                
            #     run_to_buy_done_at_frames = np.linalg.norm(robot_axit-np.array([task["buy_coordination_x"], task["buy_coordination_y"]])) / 0.1 + frame_id
            #     run_to_buy_done_at_frames = task["predict_frame_id_product_done"] if task["predict_frame_id_product_done"] > run_to_buy_done_at_frames else run_to_buy_done_at_frames
            #     run_to_buy_done_need_frames = run_to_buy_done_at_frames - frame_id
            #     run_to_sell_done_need_frames = np.linalg.norm(np.array([task["buy_coordination_x"], task["buy_coordination_y"]]) \
            #                                          - np.array([task["sell_coordination_x"], task["sell_coordination_y"]])) / 0.1
            #     needed_frames_to_finish = run_to_buy_done_need_frames + run_to_sell_done_need_frames# 0.1 # 取5米/秒 级0.1米/帧
            ...
            #     profit_per_frame = product_profit / needed_frames_to_finish
            ...
            for task in task_queue:
                ...
                to_buy_need_frames = buy_dist / 0.15 
                ...
                run_to_sell_done_need_frames = sell_dist / 0.1
                needed_frames_to_finish = to_buy_need_frames + run_to_sell_done_need_frames# 0.1 # 取5米/秒 级0.1米/帧
                if(needed_frames_to_finish > 9001-frame_id):
                    continue

                # # 加上逻辑距离优先级，优先考虑取最近的买卖高级产品的任务
                # total_dist = buy_dist
                # total_dist -= task["priority"] * 50
                # if(dist > total_dist):
                #     dist = total_dist
                #     t = task
                product_type = task["product_type"]
        ...
        if(task["task_type"] == 1): # 取任务
            ...
            if(info_robots[task["robot_id"]]["at_bench_id"] == task["sell_bench_seq"]):
                ...
                if(current_sell_bench["product_status"] == 1 and (not buy_benchs_in_quuee[current_sell_bench["bench_seq"]])):
                    ...
                    # 价格条件能在9000帧完成
                    if(res_bench and (np.linalg.norm(buy_bench_axis-np.array([res_bench["coordination_x"], res_bench["coordination_y"]])) / 0.15 + frame_id < 9001)):
